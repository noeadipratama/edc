...
class bank extends CI_Controller {
	...
	public function index()
	{
		...
		$data['listbank'] = $this->model_bank->getAllbank();
	...
	Public function Insert() 
	{
		...
		$this->model_bank->Insertbank($data);
	...
	Public function Delete($id_bank) 
	{
		...
		$this->model_bank->Deletetbank($id_bank);
		redirect('bank');
		}else{
		echo "<script>alert('Anda tidak mendapatkan access menu ini');window.location.href='javascript:history.back(-1);'</script>";	
		}
	...
	Public function FormUpdate($id_bank) 
	{
		if($this->session->userdata('login'))
        {
		$session = $this->session->userdata('login');
        ...
        $data['id_user'] = $session['id_user'];
        $data['session_level'] = $session['id_level'];
		$data['listbankselect'] = $this->model_bank->getAllbankselect($id_bank);
	...
	Public function Update() 
	{
		...
		$this->model_bank->Updatebank($id_bank, $data);
		redirect('bank');
		}
		}else{
		echo "<script>alert('Anda tidak mendapatkan access menu ini');window.location.href='javascript:history.back(-1);'</script>";	
